...
def friend_overlap(users):
    ...
    ###TODO
    result = []
    for user1 in range(0, len(users)):
        for user2 in range(0, len(users)):
            if (user1 < user2):
                name1 = users[user1]['screen_name']
                name2 = users[user2]['screen_name']
                overlap = [i for i in users[user1]['friends'] if i in users[user2]['friends']]
                result.append((name1, name2, len(overlap)))
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    """
    ###TODO
    #init hillary and donald list for storing their friends
    hillary = []
    donald = []
    for i in users:
    ...
    #convert their friend in to set and find out the overlap of them
    overlap = set(hillary) & set(donald)

    #get the overlap one
    target = list(overlap)[0]

    #lookup the name and return it
    result = robust_request(twitter, 'users/lookup', {'user_id': target})
    name = []
    for r in result:
        name.append(r['screen_name'])
    return name
...
def create_graph(users, friend_counts):
    ...
    #Create an empty undirected graph structure with no nodes and no edges
    G = nx.Graph()

    #init a list for storing screen_name and friends
    l = []

    #In the list every dict is a candidate who only has screen_name and friends. Just like projection in SQL
    for c in range(0, len(users)):
        d = {}
        d['screen_name'] = users[c]['screen_name']
        d['friends'] = users[c]['friends']
        l.append(d)

    #add each candidate as a node into graph. 
    for i in l:
        name = i['screen_name']
        friends = i['friends']
        G.add_node(name)

        #for each candidate, find out the friend who has more than one count. link friend and candidate
        for f in friends:
...
def draw_network(graph, users, filename):
    ...
    """
    ###TODO
    #retrieve only candidates' name from users
    names = []
    for i in range(0, len(users)):
        names.append(users[i]['screen_name'])

    #change labels to show only candidates' name
    label = {l: l if l in names else '' for l in graph.nodes()}

    #set the figsize
    plt.figure(figsize=(10, 10))

    #draw network using label we changed, set node size, edge width and node transparency
    nx.draw_networkx(graph, labels=label, node_size=100, width=0.1, alpha=0.5)

    #turn axis off
    plt.axis("off")

    #save figure with given file name
